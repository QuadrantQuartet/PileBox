...
struct Part {
    ...
    int pos;
    int len;
    Part() : stretchable(false), pos(0), len(0) {}
    Part(int pos, int len, bool stretchable)
        : stretchable(stretchable), pos(pos), len(len) {}
};
    ...
    if (sw > 2 && sh > 2 && dw > 0 && dh > 0) {
        ...
        int vert_stretch = 0;
        {
            ...
            for (int x = 1; x < sw - 1; x++) {
                ...
                if (isStretchableMarker(last) != isStretchableMarker(next) ||
                    x == sw - 2) {
        ...
        double vert_mul = 0;
        if (horz_stretch > 0)
            horz_mul = (double)(dw - (sw - 2 - horz_stretch)) / horz_stretch;
        if (vert_stretch > 0)
            vert_mul = (double)(dh - (sh - 2 - vert_stretch)) / vert_stretch;
        int dy0 = 0;
        int dy1 = 0;
        double vstretch = 0;
